...
port = 8000  # ssh -i /etc/ssh/ssh_host_rsa_key root@10.6.28.28
...
def scale(container_needed):
    playbook_path = "/var/lib/awx/projects/_6__gizwits_demo/demo" + containers_needed + ".yml"
    print playbook_path
    cmd_scale = "ansible-playbook " + playbook_path
    scale_result = run(cmd_scale)

req_per_container = 100
containers_needed = (statistics['portUsedCounts']/req_per_container) + 1
cmd_count_container = "docker ps | grep demo-app | wc -l"
cur_containers = run(cmd_count_container)

#if containers_needed <> cur_containers:
scale(containers_needed)
